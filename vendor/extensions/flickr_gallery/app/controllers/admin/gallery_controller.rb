class Admin::GalleryController < ApplicationController
  ...
  def index
    ...
    url = "http://api.flickr.com/services/rest/?method=flickr.photosets.getList&api_key=#{@api_key}&user_id=#{@user}"
    ...
    begin
      ...
      # return "<p>The flickr API taking too long to respond.</p>"
      flash[:error] = "The flickr API is taking too long to repsond";
    rescue OpenURI::Error => e
      # return "<p>the flickr API is experience problems with their API.</p>"
      flash[:error] = "The flickr API appears to be down at this time";
    end
